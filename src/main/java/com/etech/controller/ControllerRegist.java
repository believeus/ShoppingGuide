...
public class ControllerRegist {
	...
	private static final Log log = LogFactory.getLog(ControllerRegist.class);
	@Resource
	...
	public void ajaxComValidReg(Tshopsuser regUser,String submit ,String rpasswprd,
		...
		if (StringUtils.isEmpty(sessionUser)) {
			...
			Tshopsuser user = (Tshopsuser) etechService.findObjectByProperty(
					Tshopsuser.class, "id", regUser.getId());
			// 编辑用户名和原用户名不等
		...
		if ("submit".equals(submit)) {
			...
			regUser.setUserCreatTime(new Timestamp(System.currentTimeMillis()));
			...
			if (StringUtils.isEmpty(sessionUser)) {
				...
				Tshopsuser comUser = (Tshopsuser) etechService.findObjectById(
						Tshopsuser.class, regUser.getId());
